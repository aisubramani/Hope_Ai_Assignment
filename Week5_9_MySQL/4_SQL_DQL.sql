...
SELECT COUNT(enrollment_date) from students GROUP BY enrollment_date;
select enrollment_date, COUNT(*) as total from students GROUP BY enrollment_date;

select * from students;
select avg(marks) as total from students;
select MAX(marks) as total from students;
...
SELECT MIN(marks) as total from students;
...
INSERT INTO customers (c_name, c_country)
VALUES ("alice", "usa"),
		("bob", "canada"),
        ("charlie", "uk"),
        ("diana", "australia");
...
select * from orders;
select c_id, sum(amount) from orders GROUP BY c_id;
...
select customers.c_id, customers.c_name, orders.o_id, orders.amount 
