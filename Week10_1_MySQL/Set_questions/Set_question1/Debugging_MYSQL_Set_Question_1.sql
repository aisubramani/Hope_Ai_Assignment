-- Debugging MySQL Set Question 1 
...
--   Syntax Error Identification, Ans= Missing Comma ,
SELECT name, age FROM employees WHERE salary >= 50000;

--  Logical Error Detection, Ans= OR
SELECT * FROM orders WHERE order_date = '2023-05-01' OR order_date = '2023-05-02';

-- Performance Optimization, Ans= Create index and select only required columns
CREATE INDEX idx_amount ON sales(amount);
SELECT amount, customer_id FROM sales WHERE amount > 5000;


-- Data Type Mismatch, Ans= price = 100
SELECT * FROM products WHERE price = 100;

-- Incorrect Join Condition, Ans= d.department_name =d.department_id
SELECT e.name, d.department_name
FROM employees e
INNER JOIN departments d ON e.department_id = d.department_name;

--  Incorrect NULL Handling, Ans= "!=" = IS NOT NULL
SELECT * FROM customers WHERE customer_name IS NOT NULL;

-- Incorrect Use of Aggregate Functions, Ans= GROUP BY customer_id;
SELECT customer_id, SUM(amount)
FROM sales GROUP BY customer_id;

-- Incorrect Column Selection, Ans= customer_address =customer_Name
